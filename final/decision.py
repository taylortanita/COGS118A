...
def divide_fg(x,y):
  ...
  for i in range(row):
    if (fg_percent[i] < avg):
      above = np.concatenate((above,x[i]),axis=0)
    else:
      below = np.concatenate((below,x[i]),axis=0)
  above = above[row:,:]
  ...
  above_y = above[:,(col-1)]
  below_y = below[:,(col-1)]
  above = np.delete(above,(col-1),1)
  below = np.delete(below,(col-1),1)
  return (above,above_y,below,below_y)

def predict(x,y,x_test,y_test,D):
...
def decisions(player,team,team_iso,wins):
  (D_team,team_error) = decision(team,wins)
  (D_player,player_error) = decision(player,wins)
  (D_team_iso,team_iso_error) = decision(team_iso,wins)
